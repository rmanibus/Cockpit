...
import { Button, Input, Tag, Table, Space } from 'antd';
...
export const ListOrDictEditor: React.FC<ListProps<any>> = ({ name, list, update }: ListProps<any>) => {
  ...
  return (
    <>
      ...
      {Array.isArray(list) ? <KeyValueListTable list={list} update={update} /> : <DictTable list={list} update={update} />}
    </>
  );
};
...
export const DictTable: React.FC<ListProps<any>> = ({ list, update }: ListProps<any>) => {
